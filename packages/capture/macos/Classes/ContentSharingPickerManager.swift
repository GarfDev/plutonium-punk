...
actor CallbackStore {
    ...
    func setContentSelectedCallback(
        _ callback: @Sendable @escaping (SCContentFilter, SCStream?) -> Void
    ) {
...
class ContentSharingPickerManager: NSObject, SCContentSharingPickerObserver {
    ...
    nonisolated func contentSharingPicker(
        _: SCContentSharingPicker, didUpdateWith filter: SCContentFilter, for stream: SCStream?
    ) {
        ...
        Task { @MainActor in
            if let callback = await ContentSharingPickerManager.shared.callbackStore
                .getContentSelectedCallback()
            {
    ...
    nonisolated func contentSharingPicker(_: SCContentSharingPicker, didCancelFor stream: SCStream?)
    {
        ...
        Task { @MainActor in
            if let callback = await ContentSharingPickerManager.shared.callbackStore
                .getContentSelectionCancelledCallback()
            {
    ...
    nonisolated func contentSharingPickerStartDidFailWithError(_ error: any Error) {
        ...
        Task { @MainActor in
            if let callback = await ContentSharingPickerManager.shared.callbackStore
                .getContentSelectionFailedCallback()
            {
