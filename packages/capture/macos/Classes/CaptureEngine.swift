...
class CaptureEngine: NSObject, @unchecked Sendable, SCRecordingOutputDelegate {
    ...
    private let audioSampleBufferQueue = DispatchQueue(
        label: "com.example.apple-samplecode.AudioSampleBufferQueue")
    ...
    @MainActor func startCapture(
        ...
        return AsyncThrowingStream<CapturedFrame, any Error> { continuation in
            ...
            do {
                stream = SCStream(
                    filter: contentFilter, configuration: config, delegate: streamOutput)
    ...
    func stopCapture() async -> @Sendable (@escaping @Sendable (Result<URL, any Error>) -> Void) ->
        Void
    {
